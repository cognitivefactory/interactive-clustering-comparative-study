...
"""
* Name:         listing_envs
* Description:  A set of method to list created interactive clustering comparative study environments.
* Author:       Erwan Schild
* Created:      24/05/2021
...
def get_list_of_dataset_env_paths() -> List[str]:
    """
    A method aimed at list relative paths to all dataset environments in `../experiments/` directory.
    Dataset environments are first level subfolder of `../experiments/` and have a `config.json` file.
...
def get_list_of_experiment_env_paths() -> List[str]:
    """
    A method aimed at list relative paths to all experiment environments in `../experiments/` directory.
    Experiment environments are third level subfolder of `../experiments/` (i.e. are subfolder of clustering environments) and have a `config.json` file.
