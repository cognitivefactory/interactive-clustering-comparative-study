...
* Description:  Worker to evaluate an interactive clustering comparative study experiment.
* Author:       Erwan Schild
* Created:      24/05/2021
...
from sklearn import metrics  # Statistics management.
...
def experiment_evaluate(
    ...
    """
    A worker to evaluate an interactive clustering comparative study experiment.
    ...
    Then, go to the next step of the comparative study to plot iteration progress (performance evolution, time spent, annotation needed).
    Usage note:
    ...
    Args:
        parameters (Dict[str, Any]): A dictionary that contains several parameters. Two keys are expected in this dictionary: the experiment environment path (`"ENV_PATH"`), the comparative study progress to print (`"comparative_study_progress"`). An optional key (`"performance_goals_to_compute"`) can be added to define performance goal for iteration to highlight computation.
    ...
    with open(ENV_PATH + "config.json", "r") as file_2:
        CONFIG_EXPERIMENT = json.load(file_2)
    ...
    with open(ENV_PATH + "../../../../../dict_of_true_intents.json", "r") as file_3:
        dict_of_true_intents: Dict[str, str] = json.load(file_3)
    ...
    with open(ENV_PATH + "dict_of_clustering_results.json", "r") as file_4:
        dict_of_clustering_results: Dict[str, Dict[str, int]] = json.load(file_4)

    # Load dictionary of annotation history.
    with open(ENV_PATH + "dict_of_constraints_annotations.json", "r") as file_5:
        dict_of_constraints_annotations: Dict[
            str, List[Tuple[str, str, str]]
        ] = json.load(file_5)

    # Load dictionary of time spent.
    with open(ENV_PATH + "dict_of_computation_times.json", "r") as file_6:
        dict_of_computation_times: Dict[str, Dict[str, float]] = json.load(file_6)
    ...
    with open(ENV_PATH + "log.out", "a") as stdout_7:
        stdout_7.write(
            "\n" + "    " + comparative_study_progress + " # Evaluate clustering."
        )
    ...
    with open(ENV_PATH + "dict_of_clustering_performances.json", "w") as file_8:
        json.dump(dict_of_clustering_performances, file_8, indent=1)
    ...
    with open(ENV_PATH + "log.out", "a") as stdout_9:
        stdout_9.write(
            "\n" + "    " + comparative_study_progress + " # Find specific iteration."
        )
    ...
    with open(ENV_PATH + "dict_of_iterations_to_highlight.json", "w") as file_10:
        json.dump(dict_of_iterations_to_highlight, file_10, indent=1)
    ...
    with open(ENV_PATH + "log.out", "a") as stdout_13:
        stdout_13.write(
            "\n" + "    " + comparative_study_progress + " # Plot time spent evolution."
        )
    ...
    # Print end log.
    with open(ENV_PATH + "log.out", "a") as stdout_14:
        stdout_14.write(
            "\n"
            + "\n####################################################################################################"
        )
        ...
        stdout_14.write(
            "\n" + "### " + comparative_study_progress + " : " + "END EVALUATION"
        )
        stdout_14.write(
            "\n"
            + "####################################################################################################"
